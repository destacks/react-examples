...
// React has two ways of creating componens, class-based and function-based.
// Function-based is the modern, recommended way!!!

// JSX = "J"ava"S"cript an "X"ML

//
// Every {} in JSX is going into JavaScript-Land. In there you can use normal JS assignments.
// Examples:
//
// - a.): 1 + 1
// - b.): "Bob"
...
// React is nothing else than Client-Side JS which inhabits one element of your DOM,
...
// Reason:
//
// React does NOT operate in the real "DOM", React operates in the so called "Virtual DOM".
// In the "Virtual DOM" world, it produces one "Virtual DOM" (current) and on changes a second "Virtual DOM" (next)
// and does a difference calculation (aka diff calculation) between those (diff: current <-> next).
// And ONLY injects the differences (surgically) in the real "DOM" (changedElement.innerHTML = "I changed :D").
//
// In Principle:
//
// Real-DOM (looks at it once on startup) -> Virtual-DOM (current) -> Virtual-DOM` (aka changed, next DOM) -> diff of both VDOMs ->
// inject into Real-DOM -> on success, "next" VDOM becomes "current", and cylce repeats.
//
// It does this on EVERY change!!! If you have many changes and or frequent changes,
// this will slow down React to a hold, because of to many changes.
//
// Thumb Rules:
//
// 1.)  Creating a real new DOM in a browser (like Chrome) is very expensive. Expensive means,
//      its taking CPU cycles (aka calculation power) and therefore time.
//
// 2.)  Injecting only changes via "elemnet.innerHTML" is much cheaper, thats why React does this.
//
// 3.)  BUt it has a Virtual DOM diffing cost, thats why "Virtual DOM diffing" like React does it
//      is a dumb but good enough solution (most of the time)
//
// 4.)  Better and faster solution always diff and inject on the real "DOM".
